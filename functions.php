...
if (!function_exists('wtw_is_legacy_mailer_disabled')) {
    function wtw_is_legacy_mailer_disabled()
    {
        if (function_exists('get_field') && did_action('acf/init')) {
            $site_settings = get_field('wtw_site_settings', 'options');

            return !empty($site_settings['disable_legacy_mailer']);
        }

        return !empty(get_option('options_wtw_site_settings_disable_legacy_mailer'));
    }
}
...
if (
    file_exists(dirname(__FILE__) . '/vendor/eforms/wp-easy-forms.php')
    && wtw_is_legacy_mailer_disabled()
) {
    include_once __DIR__ . '/vendor/eforms/wp-easy-forms.php';
}
...
function add_site_scripts()
{
    ...
    wtw_enqueue( 'style', 'main', 'css/main.css', [] );
}
        ...
        while (have_rows('custom_taxonomies', 'option')) : the_row();
            register_taxonomy(
                ...
                [
                    ...
                    'public' => get_sub_field('publicly_queryable') !== '' ? get_sub_field('publicly_queryable') : true,
                    'publicly_queryable' => get_sub_field('publicly_queryable') !== '' ? get_sub_field('publicly_queryable') : true,
                    ...
                    'query_var' => true,
                    'rewrite'   => get_sub_field('publicly_queryable') 
                        ? [
                            'slug'         => get_sub_field('slug'),
                            'hierarchical' => false,
                          ]
...
function wtw_ajax_load_part_inline() {
    ...
    jQuery(function($) {
        $(document).on('click', '[data-ajax-load]', function(e) {
            ...
            var template = $btn.attr('data-ajax-load');
            var target = $btn.attr('data-target');

            if (!template || template.trim() === '' || !target || target.trim() === '') {
                return;
            }
            ...
            ajaxs('wtw_load_chunk', {
                ...
                method   : $btn.attr('data-method') || 'replace'
            });
        });
    });
    </script>
...
function wtw_enqueue( $type, $handle, $file, $deps = [], $footer = true, $attr = null ) {
