<?php
/*
Template name: Ресторан «Итиль»
*/
?>
    <!DOCTYPE html>
<html data-wf-page="686931998f03c728d6fcf3d0" data-wf-site="686931998f03c728d6fcf388" lang="ru">
	<?php get_template_part("header_block", ""); ?>
	<body>
<?php if(function_exists('get_field')) { echo get_field('body_code', 'option'); } ?>

		<?php if( have_rows('bloki_stranicy_restoran') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блоки страницы Ресторан"; $loop_field = "bloki_stranicy_restoran"; while( have_rows('bloki_stranicy_restoran') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
			<div>
				<div class="w-embed">
					<style>.w-nav-overlay {
	margin-top: 60px;
...
.nav-button:hover::after {
  width: 100%;
			...
			<?php if( have_rows('pervaya_sekciya') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Первая секция"; $loop_field = "pervaya_sekciya"; while( have_rows('pervaya_sekciya') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
				<section class="section_same-page rest" style="background-image:url('<?php $field = get_sub_field('izobrazhenie_verhnej_sekcii'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
					<div class="container-xl same-page">
						<div class="same-box">
							<h1 class="h2 trip b"><?php echo get_sub_field('zagolovok_n1') ?></h1>
				...
				<div class="padding-global">
					<div class="container-large">
						<div class="padding-section-large">
							<?php if( have_rows('bloki_restorana') ){ ?><div data-w-id="555597d4-3266-5435-f7f3-d5a7ae42b984" class="w-layout-grid layout408_component"><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блоки ресторана"; $loop_field = "bloki_restorana"; while( have_rows('bloki_restorana') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
								<?php if( have_rows('blok_restoran') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блок ресторан"; $loop_field = "blok_restoran"; while( have_rows('blok_restoran') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
									<div class="layout408_card-rest resech-img <?php echo get_sub_field('dobavit_klass') ?>">
										<div class="layout408_image-wrapper-rest"><img class="layout408_image-rest" src="<?php $field = get_sub_field('izobrazhenie'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" width="500" alt="<?php echo !empty($field['alt']) ? esc_attr($field['alt']) : ''; ?>" loading="lazy"></div>
										<div class="layout408_card-content">
											<div class="layout408_card-content-top">
												<div class="margin-bottom margin-small">
													<h3 class="heading-h3"><?php echo get_sub_field('zagolovok_n3') ?></h3>
												</div>
												<p class="paragraph"><?php echo get_sub_field('opisanie') ?></p>
											</div>
										</div>
									</div>
								</div><?php } ?><?php } ?>
			...
			<?php if( have_rows('sekciya_vse_vklyucheno') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Секция Все включено"; $loop_field = "sekciya_vse_vklyucheno"; while( have_rows('sekciya_vse_vklyucheno') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><section id="restoran-vse-vklucheno" class="section_food">
				<div class="container-xl green-cont">
					<div class="testimonial-info-four">
						<div class="wave-icon"><img loading="lazy" src="<?php echo get_template_directory_uri() ?>/images/6869365a5499af0f5aeac75f_emmaus-green-bron.svg" alt class="testimonial-image"></div>
						<div class="about-1200 rest">
							<p class="preim-text"><?php echo get_sub_field('opisanie_mini') ?></p>
							<h2 class="h2"><?php echo get_sub_field('zagolovok_n2') ?></h2>
							<p class="preim-text _18-300"><?php echo get_sub_field('opisanie_pod_zagolovkom') ?></p>
						</div>
					</div>
					<div class="food-rest_grid">
						<?php if( have_rows('blok_1') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блок 1"; $loop_field = "blok_1"; while( have_rows('blok_1') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div id="w-node-_2ffa275d-59ce-adc1-e913-3fe469c004e4-d6fcf3d0">
							<div class="food_item rest" style="background-image:url('<?php $field = get_sub_field('izobrazhenie'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
								<div>
									<p class="preim-text_black"><?php echo get_sub_field('tekst_mini') ?></p>
									<div class="h3 food rest"><?php echo get_sub_field('tekst_vydelennyj') ?></div>
								</div>
								<div class="margin-top margin-medium">
									<div class="button-group">
										<?php if(empty(get_sub_field('skryt_knopku'))) { ?><div><a href="<?php echo get_sub_field('ssylka_knopki') ?>" target="_blank" class="main-button-ht green breaking-norm w-button"><?php echo get_sub_field('knopka') ?></a></div><?php } ?>
									</div>
								</div>
							</div>
						</div><?php } ?><?php } ?>
						<?php if( have_rows('blok_2') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блок 2"; $loop_field = "blok_2"; while( have_rows('blok_2') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
							<div class="food_item rest2" style="background-image:url('<?php $field = get_sub_field('izobrazhenie'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
								<div>
									<p class="preim-text_black"><?php echo get_sub_field('tekst_mini') ?></p>
									<div class="h3 food w rest2"><?php echo get_sub_field('tekst_vydelennyj') ?></div>
								</div>
								<div class="margin-top margin-medium">
									<div class="button-group">
										<?php if(empty(get_sub_field('skryt_knopku_1'))) { ?><div><a href="<?php echo get_sub_field('ssylka_knopki_1') ?>" target="_blank" class="main-button-ht breaking-norm w-button"><?php echo get_sub_field('knopka_1') ?></a></div><?php } ?>
										<?php if(empty(get_sub_field('skryt_knopku_2'))) { ?><div><a href="<?php echo get_sub_field('ssylka_knopki_2') ?>" target="_blank" class="main-button-ht breaking-norm w-button"><?php echo get_sub_field('knopka_2') ?></a></div><?php } ?>
									</div>
								</div>
							</div>
						</div><?php } ?><?php } ?>
						<?php if( have_rows('blok_3') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блок 3"; $loop_field = "blok_3"; while( have_rows('blok_3') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
							<div class="food_item rest3" style="background-image:url('<?php $field = get_sub_field('izobrazhenie'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
								<div class="div-block-435">
									<div class="h3 food"><?php echo get_sub_field('tekst_vydelennyj') ?></div>
									<p class="preim-text_black"><?php echo get_sub_field('tekst_mini') ?></p>
								</div>
								<div class="margin-top margin-medium">
									<div class="button-group">
						...
						<?php if( have_rows('blok_4') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Блок 4"; $loop_field = "blok_4"; while( have_rows('blok_4') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div id="w-node-e4bbdd20-407d-a9e3-f3f8-323ccb4046b5-d6fcf3d0">
							<div class="food_item rest4" style="background-image:url('<?php $field = get_sub_field('blok_4'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
								<div class="div-block-435">
									<div class="h3 food"><?php echo get_sub_field('tekst_vydelennyj') ?></div>
									<p class="preim-text_black"><?php echo get_sub_field('tekst_mini') ?></p>
								</div>
...
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/restoran.js?ver=1789049048"></script></body>
</html>
