...
?>
    <!DOCTYPE html>
<html data-wf-page="686931998f03c728d6fcf43a" data-wf-site="686931998f03c728d6fcf388" lang="ru">
	<?php get_template_part("header_block", ""); ?>
	<body>
<?php if(function_exists('get_field')) { echo get_field('body_code', 'option'); } ?>

		<?php if( have_rows('stranica_akcij') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Страница Акций"; $loop_field = "stranica_akcij"; while( have_rows('stranica_akcij') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
			<div>
				<div class="w-embed">
					<style>.w-nav-overlay {
	margin-top: 60px;
...
.nav-button:hover::after {
  width: 100%;
			...
			<?php if( have_rows('sekciya_pervaya') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Секция первая"; $loop_field = "sekciya_pervaya"; while( have_rows('sekciya_pervaya') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><div>
				<section class="section_same-page-akcii" style="background-image:url('<?php $field = get_sub_field('izobrazhenie_verhnej_sekcii'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>');">
					<div class="fill-bg-sect-akcii">
						<div class="container-xl same-page">
							<div class="same-box o-nas_top-txt">
								<h1 class="h2 trip"><?php echo get_sub_field('zagolovok_n2') ?></h1>
								<p class="preim-text"><?php echo get_sub_field('mini_opisanie_akcii') ?></p>
							</div>
							<div class="reservation_block">
								<div class="fiil_block"><?php echo get_field('kod_paneli_bronirovaniya', 'options') ?></div>
								<?php if(empty(get_field('skryt_blok_pod_bronirovaniem', 'options'))) { ?><div class="news-block-vesna"><img width="70" alt src="<?php echo get_template_directory_uri() ?>/images/69ba9f4b6aabc7711e9f3031_Frame2054.png" loading="lazy" class="image-46">
									<div class="text-block-52"><?php echo get_field('tekst_ng', 'options') ?></div><img width="70" alt src="<?php echo get_template_directory_uri() ?>/images/69ba9f4b6aabc7711e9f3031_Frame2054.png" loading="lazy" class="image-46 trhg">
									<div class="button-group top-mob jfn">
										<?php if(empty(get_field('skryt_knopku_2', 'options'))) { ?><div><a href="<?php echo get_field('ssylka_knopki_ng', 'options') ?>" class="main-button-ht w-button"><?php echo get_field('knopka_ng', 'options') ?></a></div><?php } ?>
									</div>
								</div><?php } ?>
							</div>
						</div>
			...
			<?php if( have_rows('sekciya_opisanie_akcii') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Секция Описание Акции"; $loop_field = "sekciya_opisanie_akcii"; while( have_rows('sekciya_opisanie_akcii') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><section id="second-section" class="section-paiding-60">
				<div class="container-xl green-cont"><?php if(have_posts()) : ?><?php while (have_posts()) : the_post(); ?>
					<div class="about-us_grid-akcii">
						<div class="what-to-do_text-block">
							<div class="aboute-us_text-block-korp">
								<div class="rich-text-block w-richtext"><?php echo get_sub_field('tekst_akcii') ?></div>
							</div>
						</div><img class="image-31-korp" src="<?php $img = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'); echo $img[0]; ?>" width="441" alt="<?php echo get_the_excerpt(); ?>" loading="lazy" title="<?php echo get_the_title(); ?>"></div>
				<?php endwhile; ?><?php endif; ?></div>
			</section><?php } ?><?php } ?>
			<?php if( have_rows('sekciya_faq') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Секция FAQ"; $loop_field = "sekciya_faq"; while( have_rows('sekciya_faq') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><section id="red-section" class="section-akcii">
				<div class="container-xl green-cont">
					<div class="about-600-akcii">
						<h2 class="h2"><?php echo get_sub_field('zagolovok_n2') ?></h2>
					</div>
					<?php if( have_rows('dobavit_vopros') ){ ?><div class="div-block-465"><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Добавить Вопрос"; $loop_field = "dobavit_vopros"; while( have_rows('dobavit_vopros') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
						<div class="accordion2_component">
							<div data-w-id="295869b5-1dd3-6673-2924-19779ad4981a" class="accordion2_top">
								<div class="text-size-medium text-weight-bold"><strong><?php echo get_sub_field('vopros') ?></strong></div>
							...
							<div style="height:0px" class="accordion2_bottom">
								<div class="margin-bottom margin-small">
									<p><?php echo get_sub_field('otvet') ?></p>
								</div>
							</div>
						</div>
			...
			<?php if( have_rows('sekciya_drugie_akcii') ){ ?><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Секция другие акции"; $loop_field = "sekciya_drugie_akcii"; while( have_rows('sekciya_drugie_akcii') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?><section id="second-section" class="section-paiding-60">
				<div class="container-padding-20">
					<div class="container-xl green-cont">
						<div class="about-600-akcii">
							<h2 class="h2"><?php echo get_sub_field('zagolovok_n2') ?></h2>
						</div>
						<?php $current_id = get_queried_object_id(); $parent_id  = get_post_field( 'post_parent', $current_id ); ?>
						<?php if( !isset($query_args)) $query_args = array('post_type'=> 'page', 'post_parent'=> $parent_id, 'post__not_in'=> array( $current_id ), 'posts_per_page' => 6, 'orderby'=> 'date', 'order'=> 'DESC', 'post_status'=> 'publish', 'no_found_rows'  => false,); if( isset($args['paged']) && $args['paged']){ $query_args['paged'] = $args['paged']; }else{ $query_args['paged'] = get_query_var('page') ? get_query_var('page') : get_query_var('paged'); }$query = new WP_Query($query_args); if($query->have_posts()) : ?>
<div class="what-to-do_grid-korporate">
...
<?php else : ?><?php endif; unset($query_args); wp_reset_postdata(); ?>
			...
			<div><?php $current_id = get_queried_object_id(); $parent_id  = get_post_field( 'post_parent', $current_id );  ?></div>
...
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/stranica-akcii.js?ver=1789049048"></script></body>
</html>
